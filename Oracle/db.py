...
def get_pending_markets():
    conn = sqlite3.connect(DB_PATH)
    cursor = conn.cursor()
    cursor.execute("SELECT pool_id, deadline, threshold, metric_type FROM markets WHERE status = 'pending'")
...
def mark_resolved(pool_id):
    conn = sqlite3.connect(DB_PATH)
    cursor = conn.cursor()
    cursor.execute("UPDATE markets SET status = 'resolved' WHERE pool_id = ?", (pool_id,))
    conn.commit()
    conn.close()
