...
- **Transaction execution broken** - Transactions were showing "submitted" but never hitting the chain. Root causes:
  - Missing 5th input (`credits.aleo/credits` record) for the `predict` function; now explicitly fetched via `requestRecords` and attached
  - Fee too low (`0.1 ALEO`) for cross-program ZK proof execution; increased to `1.5 ALEO`
  - `privateFee: true` required two separate records; switched to public fee so users only need one private record for the stake
...
### Changed
- `Frontend/hooks/use-prediction.ts` - Rewrote transaction submission with proper record resolution, higher fee, and public fee mode
