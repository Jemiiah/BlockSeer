...
const REFUND_FEE = 1_500_000; // 1.5 ALEO in microcredits
...
export function useRefund() {
  ...
  const refundPrediction = useCallback(
    async (predictionRecord: string, onProgress?: (msg: string) => void) => {
      ...
      try {
        progress('Submitting refund transaction...');

        const result = await executeTransaction({
          program: PROGRAM_ID,
          function: 'refund_prediction',
          inputs: [predictionRecord],
          fee: REFUND_FEE,
          recordIndices: [0],
          privateFee: false,
        });
        ...
        progress(txResult.confirmed
          ? `Refund processed! TX: ${finalTxId.slice(0, 16)}...`
          : `TX submitted: ${finalTxId.slice(0, 16)}... Check explorer.`);
        setIsLoading(false);
        return true;
      } catch (e) {
        const msg = e instanceof Error ? e.message : 'Failed to refund prediction';
        setError(msg);
        setIsLoading(false);
        return false;
      }
  ...
  return { refundPrediction, isLoading, error };
}
